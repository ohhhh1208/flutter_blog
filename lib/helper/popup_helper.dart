...
class PopupMenuRouteLayout extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    // direction.
    final Size size = constraints.biggest; // 获取宽度和高度

    // 计算菜单的最大尺寸，减去指定的像素值
    final double maxWidth = size.width - (_kMenuScreenPadding * 2.0);
    final double maxHeight = size.height - (_kMenuScreenPadding * 2.0);
    final Size menuSize = Size(maxWidth, maxHeight);

    return BoxConstraints.loose(menuSize); // 使用新的 Size 对象创建 BoxConstraints
  }
