...
/*addFavourites(BuildContext context,
    {Article article, FavouriteModel model, Object tag = 'addFavourite', bool playAnim = true}) async {
  ...
  if (model.isError) {
    if (model.viewStateError.isUnauthorized) {
      if (await DialogHelper.showLoginDialog(context)) {
        ...
        if (success ?? false) {
          //登录后,判断是否已经收藏
          if (!Provider.of<UserModel>(context, listen: false).user.collectIds.contains(article.id)) {
